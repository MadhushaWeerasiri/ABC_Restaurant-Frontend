...
export default function EditOffer() {
    ...
    const loadOffer = async () => {
        try {
            const result = await Axios.get(`http://localhost:8080/api/offer/${id}`);
            setForm({
                ...result.data,
    ...
    const loadCategories = async () => {
        try {
            const result = await Axios.get(`http://localhost:8080/api/productCategory/allProducCategories`);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await Axios.put(`http://localhost:8080/api/offer/${id}`, form);
